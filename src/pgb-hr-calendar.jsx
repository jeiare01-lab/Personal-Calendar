      ...
      <div style={{ background:DS.navy, position:"sticky", top:0, zIndex:100, boxShadow:"0 2px 12px rgba(0,0,0,0.25)" }}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"12px 16px 6px" }}>
          ...
          <div style={{ display:"flex", gap:6, alignItems:"center" }}>
            ...
            {isOwner && <>
              <button onClick={()=>{ setAiPanel(p=>!p); setAiSuggestion(null); setAiError(null); }} style={{
                background: aiPanel ? DS.gold : "rgba(255,255,255,0.15)",
                border:"none", borderRadius:6, color:DS.navy,
                padding:"5px 10px", fontSize:13, fontWeight:800, cursor:"pointer"
              }} title="AI Schedule">✨</button>
        ...
        {/* ── AI PANEL ── */}
        {isOwner && aiPanel && (
          <div style={{ background:"#1A2F5A", padding:"12px 14px", borderTop:"1px solid rgba(255,255,255,0.1)" }}>
            <div style={{ fontSize:11, color:DS.gold, fontWeight:700, letterSpacing:1, marginBottom:8, textTransform:"uppercase" }}>
              ✨ AI Smart Scheduler
            </div>
            <div style={{ display:"flex", gap:8 }}>
              <input
                ...
                onKeyDown={e=>e.key==="Enter"&&!aiThinking&&runAiSchedule()}
                placeholder="e.g. Meet with AAC plant manager re: overtime policy"
                style={{
                  flex:1, background:"rgba(255,255,255,0.1)", border:"1px solid rgba(255,255,255,0.2)",
                  borderRadius:8, padding:"10px 12px", fontSize:12, color:DS.cream,
                  fontFamily:"inherit", outline:"none",
                }}
              />
              <button onClick={runAiSchedule} disabled={aiThinking||!aiInput.trim()} style={{
                background: aiThinking ? DS.steel : DS.orange,
                border:"none", borderRadius:8, color:DS.white,
                padding:"10px 14px", fontSize:12, fontWeight:700, cursor:"pointer", flexShrink:0,
              }}>{aiThinking ? "…" : "Go"}</button>
            </div>
            ...
            {/* AI Error */}
            {aiError && (
              <div style={{ marginTop:10, background:"rgba(191,54,12,0.3)", borderRadius:8, padding:"8px 10px",
                fontSize:11, color:"#FFCCBC" }}>{aiError}</div>
            )}

            {/* AI Suggestion Card */}
            {aiSuggestion && (
              <div style={{ marginTop:10, background:DS.white, borderRadius:10, padding:"12px 14px" }}>
                <div style={{ fontSize:11, color:DS.steel, fontWeight:700, marginBottom:6, textTransform:"uppercase", letterSpacing:0.5 }}>
                  ✨ AI Suggestion — pick a date &amp; confirm
                </div>

                <div style={{ fontSize:13, fontWeight:800, color:DS.navy, marginBottom:6 }}>
                  {CATEGORIES[aiSuggestion.category]?.icon} {aiSuggestion.title}
                </div>

                <div style={{ display:"flex", gap:6, flexWrap:"wrap", marginBottom:8 }}>
                  <Pill color={AFFILIATES[aiSuggestion.affiliate]?.color} bg={AFFILIATES[aiSuggestion.affiliate]?.bg}>
                    {AFFILIATES[aiSuggestion.affiliate]?.short}
                  </Pill>
                  <Pill color={CATEGORIES[aiSuggestion.category]?.color} bg="#F5F0E8">
                    {CATEGORIES[aiSuggestion.category]?.label}
                  </Pill>
                </div>
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:6, marginBottom:10 }}>
                  {[
                    { label:"Soonest available", tag:"This week" },
                    { label:"Following week",    tag:"Next week" },
                    { label:"Week after next",   tag:"+2 weeks"  },
                  ].map((opt, i) => {
                    const d = aiSlots[i] ? new Date(aiSlots[i]+"T12:00:00") : null;
                    const isSelected = selectedSlot === i;
                    return (
                      <div key={i} onClick={()=>setSelectedSlot(i)} style={{
                        display:"flex", alignItems:"center", justifyContent:"space-between",
                        padding:"8px 10px", borderRadius:8, cursor:"pointer",
                        border:`2px solid ${isSelected ? DS.orange : DS.mist}`,
                        background: isSelected ? "#FFF3E0" : "#F9F7F3",
                        ...
                        <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                          <div style={{
                            ...
                            border:`2px solid ${isSelected ? DS.orange : DS.mist}`,
                            background: isSelected ? DS.orange : "transparent",
                            display:"flex", alignItems:"center", justifyContent:"center",
                          }}>
                            {isSelected && <div style={{ width:6, height:6, borderRadius:"50%", background:DS.white }}/>}
                          </div>
                          <div>
                            <div style={{ fontSize:11, fontWeight:700, color: isSelected ? DS.orange : DS.navy }}>
                              {opt.tag}
                            </div>
                            <div style={{ fontSize:10, color:DS.steel }}>{opt.label}</div>
                          </div>
                        </div>
                        <div style={{ textAlign:"right" }}>
                          <div style={{ fontSize:12, fontWeight:800, color: isSelected ? DS.orange : DS.navy }}>
                            {d ? d.toLocaleDateString("en-PH",{month:"short",day:"numeric"}) : "—"}
                          </div>
                          <div style={{ fontSize:10, color:DS.steel }}>
                            {d ? d.toLocaleDateString("en-PH",{weekday:"long"}) : ""}
                          </div>
                        </div>
                      </div>
                    );
                ...
                {aiSuggestion.note && (
                  <div style={{ fontSize:11, color:DS.steel, marginBottom:6 }}>📝 {aiSuggestion.note}</div>
                )}

                {aiSuggestion.reasoning && (
                  <div style={{ fontSize:10, color:DS.steel, fontStyle:"italic", marginBottom:10, borderTop:`1px solid ${DS.mist}`, paddingTop:6 }}>
                    💡 {aiSuggestion.reasoning}
                  </div>
                )}

                <div style={{ display:"flex", gap:8 }}>
                  <button onClick={confirmAiSuggestion} disabled={syncing} style={{
                    flex:1, background:syncing?"#ccc":DS.orange, border:"none", borderRadius:8,
                    color:DS.white, padding:"10px", fontSize:13, fontWeight:800, cursor:"pointer"
                  }}>{syncing ? "Saving…" : "✓ Confirm & Save"}</button>
                  <button onClick={editAiSuggestion} style={{
                    background:DS.mist, border:"none", borderRadius:8,
                    color:DS.navy, padding:"10px 12px", fontSize:12, fontWeight:700, cursor:"pointer"
                  }}>Edit</button>
                  <button onClick={()=>setAiSuggestion(null)} style={{
                    background:"#FFEBEE", border:"none", borderRadius:8,
                    color:"#C62828", padding:"10px 12px", fontSize:12, fontWeight:700, cursor:"pointer"
                  }}>✕</button>
                </div>
              </div>
            )}
